...
def analyze_context(q: str = Query(..., description="文脈抽出対象のキーワード")):
    ...
    try:
        ...
        sql = """
            SELECT m.id, m.conversation_id, m.content, c.title, m.create_time
            FROM messages_fts f
            JOIN messages m ON f.rowid = m.rowid
            JOIN conversations c ON m.conversation_id = c.id
            WHERE messages_fts MATCH ?
...
def get_conversation_detail(conversation_id: str, highlight_id: str = Query(None)):
    ...
    try:
        ...
        if not conv_row:
            return {"error": f"指定されたスレッドが見つかりません。(ID: {conversation_id})"}

        # メッセージを決定論的に全件抽出します
        # conversation_id を含めることで、フロントエンドの triggerLocalAnalysis で
        # anchorMsg.conversation_id が正しく参照できるようにする
        msg_rows = conn.execute("""
            SELECT id, conversation_id, author_role, content, create_time
            FROM messages
